...
export const NavBtn = ({ id, current, icon: Icon, label, onClick }: any) => (
    <button onClick={() => onClick(id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${current === id ? 'bg-[var(--surface-high)] theme-text-main shadow-sm' : 'theme-text-muted hover:theme-bg-low hover:theme-text-main'}`}>
        <Icon className={`w-5 h-5 ${current === id ? 'text-[var(--primary)]' : ''}`} /> {label}
    </button>
);

export const StatCard = ({ title, value, color, icon }: any) => {
    const borders: any = { primary: 'border-[var(--primary)]', blue: 'border-blue-500', yellow: 'border-[var(--warning)]', emerald: 'border-[var(--success)]', red: 'border-[var(--error)]', orange: 'border-orange-500' };
    return (
...
export const ActionBtn = ({ onClick, icon, title, desc, bgIcon }: any) => (
    <button onClick={onClick} className={`p-5 rounded-2xl theme-border border theme-bg-container hover:border-[var(--primary)] transition-all text-left shadow-sm group hover:-translate-y-1`}>
...
export const FilterBtn = ({ active, onClick, label, color }: any) => {
    ...
    return <button onClick={onClick} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-colors border border-transparent shadow-sm ${colors[color]}`}>{label}</button>;
};
...
export const RoleCard = ({ title, desc, icon, color, list }: any) => {
    const borders: any = { primary: 'border-[var(--primary)]', purple: 'border-purple-500', warning: 'border-[var(--warning)]', success: 'border-[var(--success)]' };
    const textCols: any = { primary: 'text-[var(--primary)]', purple: 'text-purple-500', warning: 'text-[var(--warning)]', success: 'text-[var(--success)]' };
    
    return (
        <div className={`theme-bg-container theme-border border rounded-2xl p-6 shadow-sm`}>
            <div className="flex items-center gap-4 mb-6">
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl theme-bg-low border border-transparent border-b-4 ${borders[color]}`}>{icon}</div>
                <div>
                    <h3 className="text-lg font-bold theme-text-main">{title}</h3>
                    <p className={`text-xs font-bold uppercase tracking-wider ${textCols[color]}`}>{desc}</p>
                </div>
            </div>
            <ul className="space-y-3 text-sm theme-text-muted">
                {list.map((item: any, i: number) => <li key={i} className="flex gap-3"><span className={`font-bold ${textCols[color]}`}>•</span> {item}</li>)}
            </ul>
        </div>
    );
};

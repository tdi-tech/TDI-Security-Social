            ...
            <div className="theme-bg-container p-6 rounded-2xl border theme-border shadow-sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1"><label htmlFor="nri-total" className="text-sm font-medium theme-text-main">Total de incidencias</label><input id="nri-total" type="number" min="1" required value={formData.totalIncidencias} onChange={(e) => setFormData({...formData, totalIncidencias: parseInt(e.target.value)})} className={inputStyles} /></div>
                        <div className="space-y-1"><label htmlFor="nri-fecha" className="text-sm font-medium theme-text-main">Fecha de recepción</label><input id="nri-fecha" type="date" required value={formData.fecha} onChange={(e) => setFormData({...formData, fecha: e.target.value})} className={`${inputStyles} [color-scheme:light] dark:[color-scheme:dark]`} /></div>
                        <div className="space-y-1"><label htmlFor="nri-usuario" className="text-sm font-medium theme-text-main">Nombre / Usuario</label><input id="nri-usuario" type="text" required placeholder="@usuario o Nombre" value={formData.usuario} onChange={(e) => setFormData({...formData, usuario: e.target.value})} className={inputStyles} /></div>
                        <div className="space-y-1"><label htmlFor="nri-medio" className="text-sm font-medium theme-text-main">Medio de recepción</label><select id="nri-medio" value={formData.medio} onChange={(e) => setFormData({...formData, medio: e.target.value})} className={inputStyles}><option value="Facebook Comentario">Facebook Comentario</option><option value="TikTok">TikTok</option><option value="FB Grupos">FB Grupos</option><option value="LinkedIn">LinkedIn</option><option value="Facebook DM">Facebook DM</option><option value="Instagram DM">Instagram DM</option></select></div>
                        <div className="space-y-1"><label htmlFor="nri-campus" className="text-sm font-medium theme-text-main">Campus</label><select id="nri-campus" value={formData.campus} onChange={(e) => setFormData({...formData, campus: e.target.value})} className={inputStyles}>{['Atizapán', 'Coacalco', 'Cuautitlán Izcalli', 'Ecatepec', 'Tecamac', 'Tultepec', 'Zumpango', 'Tizayuca', 'Querétaro: la Joya', 'Querétaro: el Marqués', 'Huehuetoca', 'Chalco'].map(c => <option key={c}>{c}</option>)}</select></div>
                        <div className="space-y-1"><label htmlFor="nri-riesgo" className="text-sm font-medium theme-text-main">Nivel de Riesgo</label><select id="nri-riesgo" value={formData.riesgo} onChange={(e) => setFormData({...formData, riesgo: e.target.value})} className={inputStyles}><option value="Bajo">Bajo</option><option value="Medio">Medio</option><option value="Alto">Alto</option><option value="Critico">Crítico</option></select></div>
                        <div className="space-y-1 md:col-span-2"><label htmlFor="nri-area" className="text-sm font-medium theme-text-main">Área Responsable</label><select id="nri-area" value={formData.area} onChange={(e) => setFormData({...formData, area: e.target.value})} className={inputStyles}><option value="Legal">Legal</option><option value="Comercial - Call Center">Comercial - Call Center</option><option value="Operaciones">Operaciones</option></select></div>
                    </div>
                    <div className="space-y-1"><label htmlFor="nri-descripcion" className="text-sm font-medium theme-text-main">Descripción de incidencia</label><textarea id="nri-descripcion" required rows={3} value={formData.descripcion} onChange={(e) => setFormData({...formData, descripcion: e.target.value})} className={`${inputStyles} resize-none`}></textarea></div>
                    <div className="p-4 theme-bg-lowest border theme-border rounded-xl space-y-4">
                        ...
                        <div className="space-y-1"><label htmlFor="nri-comentarios" className="text-xs font-medium theme-text-muted">Comentarios adicionales</label><textarea id="nri-comentarios" rows={2} value={formData.comentarios} onChange={(e) => setFormData({...formData, comentarios: e.target.value})} className={`${inputStyles} resize-none`}></textarea></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1"><label htmlFor="nri-enlacePub" className="text-xs font-medium theme-text-muted">Enlace de la publicación</label><input id="nri-enlacePub" type="url" placeholder="https://" value={formData.enlacePublicacion} onChange={(e) => setFormData({...formData, enlacePublicacion: e.target.value})} className={inputStyles} /></div>
                            <div className="space-y-1"><label htmlFor="nri-enlaceDrive" className="text-xs font-medium theme-text-muted">Enlace a Google Drive</label><input id="nri-enlaceDrive" type="url" placeholder="https://drive.google.com/..." value={formData.enlaceDrive} onChange={(e) => setFormData({...formData, enlaceDrive: e.target.value})} className={inputStyles} /></div>
                        </div>
                    </div>
...
export const HistorialRRSSView = ({ showToast, isAdmin, updateRrssIncident, deleteRrssIncident }: any) => {
    ...
    // 🔥 FIX: Estado para controlar la animación del botón de descarga CSV
    ...
    // 🔥 FIX: Reseteamos la paginación a la página 1 cuando cambia el buscador o el filtro de año,
    // garantizando que los resultados en otras páginas se muestren correctamente desde el inicio.
    useEffect(() => {
        setPagePerMonth({});
    ...
    // 🔥 FIX: Agregamos simulación de carga (isExporting) para dar feedback al usuario
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 fade-in">
                    <div className="theme-bg-container rounded-2xl w-full max-w-md shadow-2xl border theme-border flex flex-col overflow-hidden">
                        ...
                        <div className="p-4 border-t theme-border flex justify-end gap-3 bg-black/5 dark:bg-white/5">
                            ...
                            {/* 🔥 FIX: Botón de descarga con estado de carga e inhabilitado */}

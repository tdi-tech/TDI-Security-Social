...
const EditorToolbar = ({ onCommand }: { onCommand: (cmd: string, val?: string) => void }) => {
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 p-2 border-b theme-border bg-black/20 text-gray-400 select-none">
            ...
            <input type="color" onChange={(e) => onCommand('foreColor', e.target.value)} className="w-6 h-6 p-0 border border-gray-600 rounded cursor-pointer bg-transparent hover:border-gray-400" title="Color de texto" />
            <div className="w-px h-4 bg-gray-700 mx-1"></div>
        ...
        <div className="max-w-4xl mx-auto space-y-6 fade-in pb-10">
            ...
            <div className="theme-bg-container p-6 rounded-2xl border theme-border shadow-sm">
                <div className="border-b theme-border pb-4 mb-6">
                    <h2 className="text-xl font-bold theme-text-main flex items-center gap-2"><Smartphone className="w-6 h-6 text-orange-500" /> Crear Incidente de RRSS</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Total de incidencias</label><input type="number" min="1" required value={formData.totalIncidencias} onChange={(e) => setFormData({...formData, totalIncidencias: parseInt(e.target.value)})} className={inputStyles} /></div>
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Fecha de recepción</label><input type="date" required value={formData.fecha} onChange={(e) => setFormData({...formData, fecha: e.target.value})} className={`${inputStyles} [color-scheme:light] dark:[color-scheme:dark]`} /></div>
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Nombre / Usuario</label><input type="text" required placeholder="@usuario o Nombre" value={formData.usuario} onChange={(e) => setFormData({...formData, usuario: e.target.value})} className={inputStyles} /></div>
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Medio de recepción</label><select value={formData.medio} onChange={(e) => setFormData({...formData, medio: e.target.value})} className={inputStyles}><option value="Facebook Comentario">Facebook Comentario</option><option value="TikTok">TikTok</option><option value="FB Grupos">FB Grupos</option><option value="LinkedIn">LinkedIn</option><option value="Facebook DM">Facebook DM</option><option value="Instagram DM">Instagram DM</option></select></div>
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Campus</label><select value={formData.campus} onChange={(e) => setFormData({...formData, campus: e.target.value})} className={inputStyles}>{['Atizapán', 'Coacalco', 'Cuautitlán Izcalli', 'Ecatepec', 'Tecamac', 'Tultepec', 'Zumpango', 'Tizayuca', 'Querétaro: la Joya', 'Querétaro: el Marqués', 'Huehuetoca', 'Chalco'].map(c => <option key={c}>{c}</option>)}</select></div>
                        <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Nivel de Riesgo</label><select value={formData.riesgo} onChange={(e) => setFormData({...formData, riesgo: e.target.value})} className={inputStyles}><option value="Bajo">Bajo</option><option value="Medio">Medio</option><option value="Alto">Alto</option><option value="Critico">Crítico</option></select></div>
                        <div className="space-y-1 md:col-span-2"><label className="text-sm font-medium theme-text-main">Área Responsable</label><select value={formData.area} onChange={(e) => setFormData({...formData, area: e.target.value})} className={inputStyles}><option value="Legal">Legal</option><option value="Comercial - Call Center">Comercial - Call Center</option><option value="Operaciones">Operaciones</option></select></div>
                    </div>
                    <div className="space-y-1"><label className="text-sm font-medium theme-text-main">Descripción de incidencia</label><textarea required rows={3} value={formData.descripcion} onChange={(e) => setFormData({...formData, descripcion: e.target.value})} className={`${inputStyles} resize-none`}></textarea></div>
                    <div className="p-4 theme-bg-lowest border theme-border rounded-xl space-y-4">
                        ...
                        <div className="space-y-1"><label className="text-xs font-medium theme-text-muted">Comentarios adicionales</label><textarea rows={2} value={formData.comentarios} onChange={(e) => setFormData({...formData, comentarios: e.target.value})} className={`${inputStyles} resize-none`}></textarea></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1"><label className="text-xs font-medium theme-text-muted">Enlace de la publicación</label><input type="url" placeholder="https://" value={formData.enlacePublicacion} onChange={(e) => setFormData({...formData, enlacePublicacion: e.target.value})} className={inputStyles} /></div>
                            <div className="space-y-1"><label className="text-xs font-medium theme-text-muted">Enlace a Google Drive</label><input type="url" placeholder="https://drive.google.com/..." value={formData.enlaceDrive} onChange={(e) => setFormData({...formData, enlaceDrive: e.target.value})} className={inputStyles} /></div>
                        </div>
                    </div>
                    <div className="pt-4 flex items-center justify-end gap-3 border-t theme-border">
                        ...
                        <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold bg-orange-600 text-white hover:bg-orange-500 transition-all disabled:opacity-50">{isSubmitting ? 'Guardando...' : <><Save className="w-5 h-5"/> Guardar Incidente</>}</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <div className="theme-bg-container rounded-2xl w-full max-w-2xl shadow-2xl border theme-border overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:border-none print:w-full print:max-w-full">
                        <div className="p-5 border-b theme-border flex justify-between items-center bg-orange-500/5 no-print">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {isAdmin && (
                                    <>
                                        <button onClick={openEdit} className="p-2 text-orange-500 hover:bg-orange-500/10 rounded-lg transition-colors" title="Editar"><Edit3 className="w-5 h-5"/></button>
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                            <form id="editRrssForm" onSubmit={(e) => {
                                ...
                                updateRrssIncident(selectedIncident.id, {
                                    ...
                                    reporteTexto: editEditorRef.current ? editEditorRef.current.innerHTML : (selectedIncident.reporteTexto || '')
                                });
                                setIsEditOpen(false);
                            }} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div className="border theme-border rounded-xl overflow-hidden bg-[var(--surface)] focus-within:border-gray-400 focus-within:ring-1 focus-within:ring-gray-400 transition-all">
                                        ...
                                        <div className="w-full p-4 theme-bg-low theme-text-main outline-none min-h-[160px] overflow-y-auto max-h-[300px] text-sm leading-relaxed custom-scrollbar wysiwyg-content" ref={editEditorRef} contentEditable dangerouslySetInnerHTML={{ __html: selectedIncident.reporteTexto || '' }} />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">

...
import { 
    ...
    RefreshCw, Globe, Clock, MessageSquare, ListChecks, History, AlertTriangle, Save, X, Edit3, Trash2
} from 'lucide-react';
...
// ==========================================
// VISTA 2: HISTORIAL DE INCIDENTES (HACKEOS)
// ==========================================
        ...
        <div className="fade-in space-y-6 pb-10">
            ...
            {filtered.length === 0 ? (
                <div className="text-center py-12 theme-bg-container rounded-2xl border theme-border">
                    <ShieldAlert className="w-12 h-12 theme-text-muted mx-auto mb-4 opacity-50" />
                    <p className="theme-text-muted">No hay incidentes de seguridad para mostrar.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((inc: any) => (
                        <div key={inc.id} onClick={() => { setSelectedIncidentId(inc.id); setDetailModalOpen(true); }} className="p-5 theme-bg-container rounded-xl border theme-border shadow-sm hover:border-[var(--primary)] transition-colors cursor-pointer group flex flex-col h-full">
...
export const DetailModal = ({ isOpen, onClose, incident, isAdmin, onToggleStatus, onEdit, onDelete }: any) => {
    ...
    return (
        <>
            {/* ESTILO OBLIGATORIO: Esta etiqueta apaga el fondo entero de la app solo al imprimir */}
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 fade-in print:static print:block print:p-0 print:bg-transparent">
                <div className="theme-bg-container rounded-2xl w-full max-w-2xl shadow-2xl border theme-border overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:border-none print:w-full print:max-w-full">
                    ...
                    <div className="p-6 overflow-y-auto custom-scrollbar print:overflow-visible flex-1">
                        ...
                        {/* EL USO DE space-y-6 GARANTIZA QUE TODAS LAS CAJAS TENGAN EL MISMO ESPACIO */}
                        <div className="space-y-6">
                            ...
                            {/* ESTA CAJA YA TIENE EL MISMO ESTILO EXACTO QUE LAS DE ARRIBA */}
                            <div>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                    <form id="editHackForm" onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 theme-bg-lowest border theme-border rounded-xl">
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Plataforma</label>
                                <input name="plataforma" type="text" required defaultValue={incident.plataforma} className={inputStyles} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Vector de Ataque</label>
                                <select value={vector} onChange={(e) => setVector(e.target.value)} className={inputStyles}>
                                    <option value="Desconocido">Desconocido</option>
                                    <option value="Phishing">Phishing</option>
                                    <option value="Malware/Troyano">Malware/Troyano</option>
                                ...
                                {vector === 'Otro' && (
                                    <input type="text" value={otroVector} onChange={(e) => setOtroVector(e.target.value)} required className={`${inputStyles} mt-2 fade-in`} placeholder="Especifica..." />
                                )}
                            </div>
                            <div className="space-y-1 md:col-span-2">
                                <label className="text-xs font-bold theme-text-muted">Descripción del Incidente</label>
                                <textarea name="descripcion" required rows={2} defaultValue={incident.descripcion} className={`${inputStyles} resize-none`}></textarea>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 p-5 theme-bg-lowest border theme-border rounded-xl">
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Vistas</label>
                                <input name="vistas" type="number" defaultValue={incident.vistas} className={inputStyles} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Interacciones</label>
                                <input name="interacciones" type="number" defaultValue={incident.interacciones} className={inputStyles} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Nivel de Impacto</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 theme-bg-lowest border theme-border rounded-xl">
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Contención Inmediata</label>
                                <textarea name="contencion" rows={3} defaultValue={incident.contencion} className={`${inputStyles} resize-none`}></textarea>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold theme-text-muted">Erradicación</label>
                                <textarea name="erradicacion" rows={3} defaultValue={incident.erradicacion} className={`${inputStyles} resize-none`}></textarea>
                            </div>
                        </div>
